...
- **Conversational follow-up (hybrid)** — after the first diagnosis, follow-up messages keep the context of what was already found. The AI decides per message: a question ("why cylinder 2?", "how do I do a leak-off test?") gets a plain-language **text reply** like talking to a mechanic; a new finding ("compression is fine", "replaced the injector, no change", a new OBD code) **updates the diagnosis** — dropping ruled-out causes and raising confirmed ones — instead of re-running blind
...
- **Known Faults panel** — on the new-case screen, once a vehicle is picked, shows a ranked list of the most frequent confirmed faults for that exact model+generation from the RAG DB (mileage-band filter, source links, one-tap "prefill hypothesis into diagnosis"). Zero AI tokens — pure DB aggregation. Backed by a canonical fault taxonomy (`gearbrain_fault_taxonomy`) and the `known-faults` edge function; cases are auto-classified on import (push-case) and via `scripts/agent/fault-taxonomy.mjs`. Repair texts are shown in the app language: the canonical `resolution` is stored in English (RAG needs one language), and localized `resolution_cs`/`resolution_de` variants are filled at import (push-case) and backfilled for existing cases via Claude (`scripts/agent/backfill-resolution-i18n.mjs`), with fallback to English when a translation is missing
