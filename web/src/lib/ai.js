...
Pole "řešení" = 2–4 nejpravděpodobnější opravné akce pro danou závadu. Každá akce = co mechanik reálně udělá v dílně (max 5 slov). Příklady: únik kapaliny → ["výměna těsnění hlavy","výměna hlavy válců","utažení šroubů hlavy"]. Ztráta výkonu turba → ["nasazení hadice zpět","výměna hadice turba","utažení spony"]. Chybné vstřikování → ["kalibrace vstřiků diagnostikou","výměna vstřikovače","vymazání adaptací ECU"]. DPF ucpán → ["nucená regenerace diagnostikou","výměna DPF filtru","čištění DPF"].
...
Field "řešení" = 2–4 most probable repair actions for this fault. Each action = what a mechanic actually does (max 5 words). Examples: coolant leak → ["replace head gasket","replace cylinder head","retorque head bolts"]. Turbo power loss → ["reattach turbo hose","replace turbo hose","tighten hose clamp"]. Faulty injection → ["recalibrate injectors via scan tool","replace injector","clear ECU adaptations"]. Clogged DPF → ["forced regeneration via scan tool","replace DPF filter","DPF cleaning"].
...
Feld "řešení" = 2–4 wahrscheinlichste Reparaturmaßnahmen für diesen Fehler. Jede Aktion = was der Mechaniker tatsächlich tut (max 5 Wörter). Beispiele: Kühlmittelverlust → ["Zylinderkopfdichtung ersetzen","Zylinderkopf ersetzen","Kopfschrauben nachziehen"]. Turbo-Leistungsverlust → ["Turboschlauch wieder anschließen","Turboschlauch ersetzen","Schlauchschelle nachziehen"]. Fehlerhafte Einspritzung → ["Injektoren mit Diagnosegerät kalibrieren","Injektor ersetzen","ECU-Adaptionen löschen"]. DPF verstopft → ["Zwangsregeneration per Diagnosegerät","DPF-Filter ersetzen","DPF-Reinigung"].
