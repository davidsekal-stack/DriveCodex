...
const ACTIVE_EU_BRANDS = VEHICLE_CATALOG.filter((entry) => entry.active);
...
const ACTIVE_US_BRANDS = VEHICLE_CATALOG_US
  .filter((entry) => entry.active)
  .filter((entry) => isExplicitUsBrand(entry.brand) || !EU_BRAND_SET.has(entry.brand.toLowerCase()));
...
function getBrandSection(brand) {
  return isExplicitUsBrand(brand) || !EU_BRAND_SET.has((brand ?? "").toString().toLowerCase()) ? "US" : "EU";
}
