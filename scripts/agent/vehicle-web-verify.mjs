...
export function buildVerifyPrompt({ brand, model, market }) {
...
export async function verifyVehicle(candidate, { llm = runLlm } = {}) {
  let raw;
  try {
    raw = await llm('catalog-verify', buildVerifyPrompt(candidate), { allowedTools: ['WebSearch'], timeoutMs: VERIFY_TIMEOUT_MS });
  } catch (err) {
