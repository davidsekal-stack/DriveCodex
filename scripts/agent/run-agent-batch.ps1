...
"@
  ...
  # ── Step 4: recall watchdog — denní kontrola, jestli verifikátor nezamítá moc přísně (NON-FATAL) ──
  # Nezávislá křížová kontrola (Claude, jiný dodavatel než DeepSeek-verifikátor)
  # přehodnotí vzorek čerstvě zamítnutých případů. Skript se sám hradí na 1×/den
  # po noční vlně (RECALL_AUDIT_HOUR), takže volání každou dávku je levný no-op.
  # Potřebuje --experimental-sqlite (čte lokální agent.db). Selhání NESMÍ ovlivnit
  # crawl běh — jen se zaloguje. Když křížová kontrola překročí práh, skript zapíše
  # recall-alert.txt; ten tady zrcadlíme na desktopový marker (a uklízíme ho).
  $recallWatchdog = Join-Path $agentDir 'recall-watchdog.mjs'
  if (Test-Path $recallWatchdog) {
    Write-LogLine -Path $logPath -Message "Running verifier recall watchdog..."
    $rwOut = [System.IO.Path]::GetTempFileName()
    $rwErr = [System.IO.Path]::GetTempFileName()
    try {
      $rwProc = Start-Process `
        -FilePath $nodeExe `
        -ArgumentList @('--experimental-sqlite', $recallWatchdog) `
        -WorkingDirectory $repoRoot `
        -NoNewWindow `
        -Wait `
        -PassThru `
        -RedirectStandardOutput $rwOut `
        -RedirectStandardError $rwErr

      foreach ($rwStream in @($rwOut, $rwErr)) {
        if (-not (Test-Path $rwStream)) { continue }
        $rwContent = Get-Content $rwStream -Raw
        if ([string]::IsNullOrWhiteSpace($rwContent)) { continue }
        Add-Content -Path $logPath -Value $rwContent.TrimEnd("`r", "`n")
      }
      Write-LogLine -Path $logPath -Message ("Recall watchdog exit_code={0}." -f [int]$rwProc.ExitCode)
    } catch {
      Write-LogLine -Path $logPath -Message ("WARN: recall watchdog failed ({0}); ignoring." -f $_.Exception.Message)
    } finally {
      Remove-Item $rwOut, $rwErr -ErrorAction SilentlyContinue
    }

    # Mirror the watchdog's alert file to a Desktop marker (same pattern as the
    # stall alarm). Present alert file → ensure marker; absent → remove marker.
...
(Vytvoril: scripts\agent\run-agent-batch.ps1)
"@
          Set-Content -Path $recallMarker -Value $recallMarkerText -Encoding utf8
