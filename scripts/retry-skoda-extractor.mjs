...
function usage(exitCode = 1) {
  console.log("Usage: node scripts/retry-skoda-extractor.mjs <to_review_dir> <out_dir>");
...
async function fetchThreadAsText({ url, forumTitle = "", sleepMs = DEFAULT_SLEEP_MS }) {
  ...
  for (let i = 0; i < pageItems.length; i++) {
    posts.push(...extractPostsFromText(pageItems[i].html, i + 1));
  }
...
async function main() {
  ...
  const extractorReviews = [];
  for (const fileName of files) {
    const review = await readJson(path.join(reviewDir, fileName));
    if (review?.stage === "extractor") extractorReviews.push({ fileName, review });
  }

  const rows = [];
  const promoted = [];

  for (let index = 0; index < extractorReviews.length; index++) {
    const { fileName, review } = extractorReviews[index];
    ...
    const extractorPrompt = [
      ...
      "- Ignore advice-only replies, retrofit/install-only topics, maintenance how-to topics, and speculative fixes.",
      "- Use forum context if it explicitly identifies the Škoda model or generation.",
      ...
      `- The classifier already believes at least one resolved case exists; only return it if the same-user fault -> repair chain is explicit in the thread text.`,
      "",
      "Forum thread text:",
      thread.text,
    ].join("\n");
    ...
    let finalReason = "Retry extractor returned no usable case.";
    ...
    if (Array.isArray(extracted) && extracted.length > 0) {
      for (const rawItem of extracted) {
        ...
        if (rebuilt.isReady) {
          ...
          finalReason = "Retry extractor produced a valid READY case.";
        ...
        finalReason = rebuilt.candidate?.vehicle_model
          ? "Retry extractor produced a case, but strict READY validation still failed."
          : "Retry extractor produced a case, but model mapping is still unresolved.";
      }
    }
    ...
    if (DEFAULT_SLEEP_MS > 0 && index < extractorReviews.length - 1) {
  ...
  const summary = {
    ...
    total_extractor_reviews: extractorReviews.length,
  ...
  await writeJsonl(path.join(outDir, `retry_extractor_triage_${stamp}.jsonl`), rows);
  await writeCsv(path.join(outDir, `retry_extractor_triage_${stamp}.csv`), rows, [
    "file",
    "thread_title",
    "thread_url",
    "parent_forum_title",
  ...
  await writeJsonl(path.join(outDir, `retry_extractor_promoted_${stamp}.jsonl`), promoted);
  await fs.writeFile(path.join(outDir, `retry_extractor_summary_${stamp}.json`), `${JSON.stringify(summary, null, 2)}\n`, "utf8");

  console.log(`Retried ${extractorReviews.length} extractor review item(s). Promoted ${promoted.length} READY candidate(s) into: ${outDir}`);
}
